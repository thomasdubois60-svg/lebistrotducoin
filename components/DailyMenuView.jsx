...
export default function DailyMenuView({daily,introduction='',phone='',phoneHref='',photo}) {
 ...
 return <div className={`${styles.view} ${styles[theme]}`} data-daily-theme={theme}>
  ...
  <header className={styles.hero}>
   <span className={styles.eyebrow}>{t("Le Bistrot Du Coin · À notre table")}</span>
  ...
  <div className={styles.container}>
   {daily.formulas.length>0&&<section className={styles.formulas} aria-label={t("Nos formules")}>
    <div className={styles.formulaIntro}><span className={styles.eyebrow}>{t("Le plaisir de déjeuner")}</span><h2>{t("Nos formules")}</h2><p>{t("Sur place ou à emporter")}</p></div>
    <div className={styles.formulaList}>{daily.formulas.map((f,i)=><div className={styles.formula} key={i}><strong>{f.name}</strong>{f.description&&<small>{f.description}</small>}<span className={styles.formulaPrice}>{f.price}</span>{f.takeawayPrice&&<small>{t("À emporter : ")}{f.takeawayPrice}</small>}</div>)}</div>
   </section>}
   <nav className={styles.navigation} aria-label={t("Parcourir le menu du jour")}>{groups.filter(([key])=>daily[key].length>0).map(([key,title])=><a key={key} href={`#${id}-${key}`}>{title}</a>)}{hasSuggestion&&<a href={`#${id}-chef`}>{t("Suggestion du chef")}</a>}</nav>
   <div className={styles.grid}>
    ...
    {hasSuggestion&&<aside id={`${id}-chef`} className={styles.suggestion}>
     <span className={styles.eyebrow}>{t("Le petit mot de la cuisine")}</span><p className={styles.chefLabel}>{t("Suggestion du chef")}</p>
   ...
   {phone&&<footer className={styles.contact}><div><h2>{t("Une table pour aujourd’hui ?")}</h2><p>{t("Pour connaître le menu exact du jour, appelez-nous.")}</p></div><a href={`tel:${phoneHref||phone.replace(/[^+\d]/g,'')}`}>{phone}<span aria-hidden="true"> ↗</span></a></footer>}
  </div>
 </div>;
}
