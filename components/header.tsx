...
export function Header(){
 ...
 return <><header className="site-header"><div className="container nav-wrap"><Link href="/" className="brand" onClick={()=>setOpen(false)}><span className="brand-mark">LBDC</span><span><strong>Le Bistrot</strong><small>Du Coin</small></span></Link><button className="menu-button" aria-label="Ouvrir le menu" onClick={()=>setOpen(!open)}>☰</button><nav className={open?'nav open':'nav'}>{links.map(([href,label])=><Link key={href} href={href} className={pathname===href?'active':''} onClick={()=>setOpen(false)}>{label}</Link>)}<a className="button compact" href={`tel:${general.phoneHref}`}>Réserver</a></nav></div></header>{showClosure&&<div className="closure-banner">{general.closureMessage}</div>}</>
}
