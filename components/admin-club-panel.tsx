...
type Member = {
  id:string; first_name:string; email:string; birthday:string|null; email_marketing:boolean; notification_interest:boolean;
...
export function AdminClubPanel({ password }: { password: string }) {
  ...
  const filtered=useMemo(()=>{const q=query.trim().toLowerCase();return q?members.filter(member=>`${member.first_name} ${member.email}`.toLowerCase().includes(q)):members},[members,query])
  ...
  return <section className="club-admin-section">
    ...
    <label>Rechercher un membre<input value={query} onChange={event=>setQuery(event.target.value)} placeholder="Prénom ou e-mail"/></label>
    <p className="admin-help">{message}</p>
    {filtered.length>0&&<div className="club-member-list">{filtered.map(member=><article key={member.id} className="club-member-card"><div><strong>{member.first_name}</strong><a href={`mailto:${member.email}`}>{member.email}</a></div><div className="club-member-meta"><span>{member.email_marketing?'E-mail accepté':'Pas d’e-mail promo'}</span><span>{member.notification_interest?'Notifications souhaitées':'Notifications non demandées'}</span><span>{member.loyalty_points} passage{member.loyalty_points>1?'s':''}</span></div></article>)}</div>}
  </section>
}
